...
if (!file.exists("savedResults.rda")) {
        ...
        bestParams <- results[which.max(results[,3]),]
        mtry <- bestParams["mtry"]
        ntree <- bestParams["ntree"]
        bootstrapSize <- bestParams["bootstrapSize"]
        ...
        trainIndices <- createDataPartition(trainingClasse, 
                                      p = bootstrapSize / length(trainingClasse), 
                                      list = FALSE)
  ...
  save(results, bestParams, basicModel, file = "savedResults.rda")
...
<li>sample tree size - `r bootstrapSize`</li>
</ul>
